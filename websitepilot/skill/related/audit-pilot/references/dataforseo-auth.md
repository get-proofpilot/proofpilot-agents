...
curl -s -X POST "https://backend.composio.dev/api/v3/tools/execute/DATAFORSEO_GET_KW_GOOGLE_ADS_KW_FOR_KW_LIVE" \
  -H "x-api-key: ***REDACTED_COMPOSIO_KEY***" \
  -H "Content-Type: application/json" \
  -d '{"arguments": {"keywords": ["electrician mesa az","plumber mesa az"], "location_code": 1014226, "language_code": "en", "sort_by": "search_volume"}, "entity_id": "pg-test-b1bcf597-d606-458b-84a8-4dc6ba8bdff9"}'
```
...
curl -s -X POST "https://backend.composio.dev/api/v3/tools/execute/DATAFORSEO_GET_DATAFORSEO_LABS_GOOGLE_TOP_SEARCHES_LIVE" \
  -H "x-api-key: ***REDACTED_COMPOSIO_KEY***" \
  -H "Content-Type: application/json" \
  -d '{"arguments": {"location_code": 2840, "language_code": "en", "limit": 20, "order_by": ["keyword_info.search_volume,desc"]}, "entity_id": "pg-test-b1bcf597-d606-458b-84a8-4dc6ba8bdff9"}'
```
...
curl -s -X POST "https://backend.composio.dev/api/v3/tools/execute/DATAFORSEO_POST_DATAFORSEO_LABS_BULK_KEYWORD_DIFFICULTY_LIVE" \
  -H "x-api-key: ***REDACTED_COMPOSIO_KEY***" \
  -H "Content-Type: application/json" \
  -d '{"arguments": {"keywords": ["electrician mesa az","plumber mesa az"], "location_code": 1014226, "language_code": "en"}, "entity_id": "pg-test-b1bcf597-d606-458b-84a8-4dc6ba8bdff9"}'
```
...
curl -s -X POST "https://backend.composio.dev/api/v3/tools/execute/DATAFORSEO_CREATE_SERP_GOOGLE_ORGANIC_TASK_POST" \
  -H "x-api-key: ***REDACTED_COMPOSIO_KEY***" \
  -H "Content-Type: application/json" \
  -d '{"arguments": {"tasks": [{"keyword": "electrician mesa az", "location_code": 1014226, "language_code": "en", "depth": 20}]}, "entity_id": "pg-test-b1bcf597-d606-458b-84a8-4dc6ba8bdff9"}'
...
curl -s -X POST "https://backend.composio.dev/api/v3/tools/execute/DATAFORSEO_GET_SERP_GOOGLE_ORGANIC_TASK_ADVANCED_BY_ID" \
  -H "x-api-key: ***REDACTED_COMPOSIO_KEY***" \
  -H "Content-Type: application/json" \
  -d '{"arguments": {"id": "TASK_ID_HERE"}, "entity_id": "pg-test-b1bcf597-d606-458b-84a8-4dc6ba8bdff9"}'
```
...
os.environ['COMPOSIO_API_KEY'] = '***REDACTED_COMPOSIO_KEY***'
from composio import ComposioToolSet

DFS_CONNECTION = "d69d7e6d-756f-4654-93ea-f981bb7528fc"
ENTITY = "pg-test-b1bcf597-d606-458b-84a8-4dc6ba8bdff9"
...
- If any slug returns 404, search for updated name before failing:
  `curl -s "https://backend.composio.dev/api/v3/tools?search=DATAFORSEO+keyword&limit=20" -H "x-api-key: ***REDACTED_COMPOSIO_KEY***"`
